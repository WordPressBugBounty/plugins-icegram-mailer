...
if ( ! class_exists( 'Icegram_Mailer_ESS_Mailer' ) ) {
	/**
	 * Class Icegram_Mailer_ESS_Mailer
	 *
	 * @since 5.6.0
	 */
	class Icegram_Mailer_ESS_Mailer extends Icegram_Mailer_Base_Mailer {
		...
		public function set_mailer_data() {
			...
			if ( ES()->is_const_defined( 'icegram', 'api_key' ) ) {
				...
				$this->api_key = ! empty( $ess_data['api_key'] ) ? $ess_data['api_key'] : '';
			}

			if ( empty( $this->api_key ) ) {
				return $this->do_response( 'error', __( 'API key is empty.', 'icegram-mailer' ) );
			}
		...
		public function add_into_batch( $email, $merge_tags = array(), $message = null ) {
			...
			$name       = isset( $merge_tags['name'] ) ? $merge_tags['name'] : '';
			$first_name = isset( $merge_tags['first_name'] ) ? $merge_tags['first_name'] : '';
			$last_name  = isset( $merge_tags['last_name'] ) ? $merge_tags['last_name'] : '';
			$list_name  = isset( $merge_tags['list_name'] ) ? $merge_tags['list_name'] : '';
			$hash       = isset( $merge_tags['hash'] ) ? $merge_tags['hash'] : '';
			 ...
			 */
			$contact_id  = isset( $merge_tags['contact_id'] ) ? $merge_tags['contact_id'] : '';
			$campaign_id = isset( $merge_tags['campaign_id'] ) ? $merge_tags['campaign_id'] : '';
			$message_id  = isset( $merge_tags['message_id'] ) ? $merge_tags['message_id'] : '';
			$list_ids    = isset( $merge_tags['list_ids'] ) ? $merge_tags['list_ids'] : '';
			...
			$contact_data = apply_filters( 'icegram_mailer_contact_mail_data', $contact_data, $merge_tags );
		...
		public function convert_es_tags_to_mailer_tags( $string = '' ) {
			...
			$mailer_tags_mapping = apply_filters( 'icegram_mailer_mailer_tags_mapping', $mailer_tags_mapping );
		 ...
		 * Send Email
		 *
		 * @param Icegram_Mailer_Message $message
		 *
		 * @return bool|WP_Error
		 */
		public function send( $message ) {

			$icegram_mailer_ess_data = Icegram_Mailer_Account::get_ess_data();

			$this->api_key = ! empty( $icegram_mailer_ess_data['api_key'] ) ? $icegram_mailer_ess_data['api_key'] : '';
			...
			$this->set_reply_to( $message->reply_to_email, $message->reply_to_name );
			...
			*/
			...
			if ( ! is_wp_error( $response ) ) {
				$body = ! empty( $response['body'] ) && icegram_mailer_is_valid_json( $response['body'] ) ? json_decode( $response['body'], true ) : '';
				if ( ! empty( $body ) ) {
					...
					if ( 'success' === $status ) {
						...
						$ess_data = Icegram_Mailer_Account::get_ess_data();
						$ess_data['next_reset']      = isset( $body['expires_at'] ) ? $body['expires_at'] : $ess_data['next_reset'];
						$ess_data['allocated_limit'] = isset( $body['allocated_limit'] ) ? $body['allocated_limit'] : $ess_data['allocated_limit'];
						$ess_data['used_limit'][$current_month]      = isset( $body['used_limit'] ) ? $body['used_limit'] : $ess_data['used_limit'][$current_month];
						...
						if ( ! empty( $body['message'] ) ) {
							...
							$error_message = __( 'An unknown error has occured. Please try again later.', 'icegram-mailer' );
						}
					}
		...
		public function set_attachments( $attachments ) {
			...
			foreach ( $attachments as $attachment_name => $attachment_path ) {
				...
				$attachment_name = ! empty( $attachment_name ) && is_string( $attachment_name ) ? $attachment_name : wp_basename( $attachment_path );

				$data[] = array(
					'content'     => base64_encode( $file ), // string, 1 character.
					'type'        => $filetype, // string, no ;, no CRLF.
					'filename'    => $attachment_name, // required string, no CRLF.
		...
		public function set_ess_metadata() {
			...
			$plan                 = Icegram_Mailer_Account::get_plan();
			$premium_plans        = array( 'pro', 'max' );
			$is_premium_plan      = in_array( $plan, $premium_plans, true );
			$ess_metadata         = array();
			$add_ess_footer_image = true;
			$footer_image_url     = trailingslashit( ICEGRAM_MAILER_PLUGIN_URL ) . 'admin/images/ess-footer-image.png';

			if ( $is_premium_plan && ! Icegram_Mailer_Account::is_ess_branding_enabled() ) {
